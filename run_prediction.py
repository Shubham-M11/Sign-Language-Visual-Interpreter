...
class Application:
    ...
    def __init__(self):
        ...
        model_path = resource_path('cnn8grps_rad1_model.h5')
        ...
        self.T.grid(row=0, column=0, pady=10, sticky='n')

        # Create a main frame to hold the two video panels and the text output
        main_frame = tk.Frame(self.root)
        main_frame.grid(row=1, column=0, sticky='nsew', padx=10, pady=10)
        main_frame.grid_rowconfigure(0, weight=1) # Row for video panels
        main_frame.grid_rowconfigure(1, weight=0) # Row for text output
        main_frame.grid_columnconfigure(0, weight=1) # Left video panel
        main_frame.grid_columnconfigure(1, weight=1) # Right processed panel

        # Left Panel (Live Video Feed)
        self.panel = tk.Label(main_frame)
        self.panel.grid(row=0, column=0, sticky='nsew', padx=5)

        # Right Panel (Processed Hand Image)
        self.panel2 = tk.Label(main_frame)
        self.panel2.grid(row=0, column=1, sticky='nsew', padx=5)

        # Frame for text outputs (Character, Sentence)
        output_frame = tk.Frame(main_frame)
        output_frame.grid(row=1, column=0, columnspan=2, sticky='ew', pady=10)
        output_frame.grid_columnconfigure(1, weight=1)

        # Current Character Display
        self.T1 = tk.Label(output_frame, text="Character:", font=("Courier", 20, "bold"))
        self.T1.grid(row=0, column=0, sticky='w')
        self.panel3 = tk.Label(output_frame, font=("Courier", 20))
        self.panel3.grid(row=0, column=1, sticky='w')

        # Sentence Display
        self.T3 = tk.Label(output_frame, text="Sentence:", font=("Courier", 20, "bold"))
        self.T3.grid(row=1, column=0, sticky='w')
        self.panel5 = tk.Label(output_frame, font=("Courier", 20), wraplength=1000, justify='left')
        self.panel5.grid(row=1, column=1, sticky='w')
        
        # Frame for Suggestions and controls
        bottom_frame = tk.Frame(self.root)
        bottom_frame.grid(row=2, column=0, sticky='ew', padx=10, pady=10)
        bottom_frame.grid_columnconfigure(1, weight=1) # Make suggestion area expandable
        
        self.T4 = tk.Label(bottom_frame, text="Suggestions:", fg="red", font=("Courier", 20, "bold"))
        self.T4.grid(row=0, column=0, sticky='w')
        
        # Frame to hold the suggestion buttons in a row
        suggestion_buttons_frame = tk.Frame(bottom_frame)
        suggestion_buttons_frame.grid(row=0, column=1, sticky='ew')
        
        self.b1 = tk.Button(suggestion_buttons_frame, font=("Courier", 15))
        self.b1.pack(side='left', padx=5, expand=True, fill='x')
        self.b2 = tk.Button(suggestion_buttons_frame, font=("Courier", 15))
        self.b2.pack(side='left', padx=5, expand=True, fill='x')
        self.b3 = tk.Button(suggestion_buttons_frame, font=("Courier", 15))
        self.b3.pack(side='left', padx=5, expand=True, fill='x')
        self.b4 = tk.Button(suggestion_buttons_frame, font=("Courier", 15))
        self.b4.pack(side='left', padx=5, expand=True, fill='x')

        # Frame for Clear and Speak buttons
        control_buttons_frame = tk.Frame(bottom_frame)
        control_buttons_frame.grid(row=0, column=2, sticky='e')

        self.clear = tk.Button(control_buttons_frame, text="Clear", font=("Courier", 15), command=self.clear_fun)
        self.clear.pack(side='left', padx=5)

        self.speak = tk.Button(control_buttons_frame, text="Speak", font=("Courier", 15), command=self.speak_fun)
        ...
        try:
            ...
            if cv2image.any:
                ...
                if hands[0]:
                    ...
                    white_path = resource_path("white.jpg")
            ...
            if hands:
                ...
                white = cv2.imread("D:\Sign Language Visual Interpreter\white.jpg")
